...
export class GameMap {
    ...
    canSpawn(type: string, pos: Vec2, ori: number, scale = 1): boolean {
        ...
        // Prevent buildings from spawning on lake center islands
        if (!def.terrain?.lakeCenter) {
            for (let i = 0; i < this.lakes.length; i++) {
                const lake = this.lakes[i];
                const dx = pos.x - lake.center.x;
                const dy = pos.y - lake.center.y;
                if (dx * dx + dy * dy < lake.innerRad * lake.innerRad) {
                    return false;
                }
            }
        }
