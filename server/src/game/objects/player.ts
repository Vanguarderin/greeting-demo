...
export class Player extends BaseGameObject {
    ...
    dropArmor(item: string, armorDef: LootDef): boolean {
        ...
        // Remove role perks when dropping a role-granting helmet
        if (armorDef.type === "helmet") {
            const helmetDef = GameObjectDefs[item] as HelmetDef;
            if (helmetDef?.role) {
                const roleDef = GameObjectDefs[helmetDef.role] as RoleDef;
                if (roleDef?.perks) {
                    for (const perkOrPerkFunc of roleDef.perks) {
                        const perkType =
                            perkOrPerkFunc instanceof Function
                                ? perkOrPerkFunc()
                                : perkOrPerkFunc;
