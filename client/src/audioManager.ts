...
export class AudioManager {
    ...
    update(dt: number) {
        // Clear out finished sounds using swap-and-pop (O(1) per removal)
        let i = 0;
        while (i < this.soundInstances.length) {
    ...
    playSound(sound: string, options = {} as Partial<Options>) {
        ...
        if (a && (!this.mute || options.forceStart)) {
            ...
            if (options.channel != "activePlayer" && options.soundPos) {
                const diffX = this.cameraPos.x - options.soundPos.x;
                const diffY = this.cameraPos.y - options.soundPos.y;
                ...
                // Early exit: if beyond max range, skip entirely
                if (distSq > rangeSq && !options.ignoreMinAllowable) {
                    return null;
                }

                const dist = Math.sqrt(distSq);
                const distNormal = math.clamp(dist / range, 0, 1);
                ...
                if (
                    ...
                    const stereoNorm = math.clamp((diffX / range) * -1, -1, 1);
